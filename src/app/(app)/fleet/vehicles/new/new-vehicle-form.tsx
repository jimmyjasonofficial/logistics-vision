...
export function NewVehicleForm({ drivers }: { drivers: Employee[] }) {
  ...
  async function onSubmit(data: VehicleFormValues) {
    ...
    const result = await createVehicleAction(vehicleData as any); 
    setLoading(false);

    if (result.success && result.vehicleId) {
      toast({
        title: 'Vehicle Created',
        description: `Vehicle ${data.model} has been added successfully.`,
      });
      router.push(`/fleet/vehicles/${result.vehicleId}`);
    } else {
      toast({
        variant: 'destructive',
        ...
        <form onSubmit={form.handleSubmit(onSubmit)}>
          <Card>
            <CardHeader>
                ...
                <CardDescription>Enter the specifications and status for the new vehicle.</CardDescription>
            </CardHeader>
          ...
          <div className="flex justify-end gap-2 mt-6">
            <Button type="button" variant="outline" onClick={() => router.push('/fleet/vehicles')} disabled={loading}>Cancel</Button>
