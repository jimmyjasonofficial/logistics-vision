  ...
  while (formData.has(`lineItems[${index}].description`)) {
    lineItems.push({
      ...
      unitPrice: parseFloat(
        formData.get(`lineItems[${index}].unitPrice`) as string
      ),
      discount: parseFloat(
        formData.get(`lineItems[${index}].discount`) as string
      ),
      account: formData.get(`lineItems[${index}].account`) as
        | string
        | undefined,
      taxRate: formData.get(`lineItems[${index}].taxRate`) as
        | string
        | undefined,
    });
    index++;
...
export async function createInvoiceAction(
  ...
  try {
    ...
    const invoiceData: InvoiceData = {
      ...
      status: formData.get("status") === "Draft" ? "Draft" : "Unpaid",
    };
