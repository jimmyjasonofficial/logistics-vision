...
import { getInvoiceById, type InvoiceWithUrl } from '@/services/invoice-service';
...
export default function EditInvoicePage() {
  ...
  useEffect(() => {
    async function fetchInvoice() {
        ...
        const [fetchedInvoice, fetchedCustomers] = await Promise.all([
        ...
        if (fetchedInvoice) {
            ...
            form.reset({
                ...fetchedInvoice,
                reference: fetchedInvoice.reference || '',
            });
        }
  ...
  async function onSubmit(data: InvoiceFormValues) {
    ...
    if (result.success) {
      toast({
        ...
        description: `Invoice ${invoiceId} has been updated successfully.`,
      });
    ...
    <div className="flex-1 space-y-4">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
            <Button type="button" variant="outline" onClick={() => router.push(`/accounting/invoices/${invoiceId}`)} disabled={loading}>Cancel</Button>
            ...
            <CardContent className="pt-6">
              <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
                ...
                <FormField control={form.control} name="dueDate" render={({ field }) => (
                  <FormItem><FormLabel>Due Date</FormLabel><FormControl><Input type="date" {...field} disabled={loading} /></FormControl><FormMessage /></FormItem>
                )} />
                ...
                <FormField control={form.control} name="reference" render={({ field }) => (
                  <FormItem><FormLabel>Reference</FormLabel><FormControl><Input placeholder="PO-123" {...field} value={field.value ?? ''} disabled={loading} /></FormControl><FormMessage /></FormItem>
                )} />
              </div>
          ...
          <div className="flex flex-col md:flex-row justify-between items-start gap-4">
            ...
            <div className="w-full max-w-sm space-y-2 self-end">
                ...
                <div className="flex justify-between"><span className="text-muted-foreground">Total tax</span><span className="font-medium">${totalTax.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span></div>
                 <div className="flex justify-between text-muted-foreground"><span className="">Credit available</span><span className="font-medium">$0.00</span></div>
                <Separator />
          ...
          <Card>
            <CardContent className="pt-6">
               <FormItem>
                  ...
                  {invoice.attachmentUrl ? (
                      <div className="flex items-center gap-4 p-2 border rounded-md bg-muted/50">
                          ...
                          <Button asChild variant="secondary" size="sm">
                              <Link href={invoice.attachmentUrl} target="_blank"><Download className="mr-2 h-4 w-4" /> Download</Link>
                          </Button>
                      </div>
                  ) : (
                  ...
                  <FormDescription>
                    {invoice.attachmentUrl ? 'Uploading a new file will replace the existing one.' : 'Attach a Bill of Lading or other document.'}
                  </FormDescription>
              </FormItem>
            </CardContent>
          </Card>
